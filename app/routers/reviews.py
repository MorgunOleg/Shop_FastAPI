...
from app.auth import allow_buyer, allow_buyer_and_admin
from app.services.products import update_product_rating
...
async def create_review(
        ...
        current_user: UserModel = Depends(allow_buyer)
):
    """
    Добавление нового отзыва на товар текущим покупателем (только для 'buyer').
    """
    ...
    if existing_review:
        raise HTTPException(
            ...
            detail="Review already exists. You can update your previous review."
        )
    ...
    # Сохраняем отзыв и рассчитываем средний рейтинг
    await db.flush()
...
async def delete_review(
        review_id: int,
        db: AsyncSession = Depends(get_async_db),
        current_user: UserModel = Depends(allow_buyer_and_admin)
):
    """
    Выполняет мягкое удаление отзыва, если он принадлежит текущему покупателю, или это админ.
    """
    # Ищем отзыв
    ...
    # Проверка прав (админ или автор отзыва)
    ...
    # Сбрасываем/пересчитываем рейтинг товара
    await update_product_rating(db, product_id=review.product_id)
